...
import type { IPoint, IUnit, IUnitAssignment } from '@/entities/unit'
import type { IPositionInput } from '@/entities/position'
import { ConfirmDialog, If, Spinner } from '@/shared/ui'
import { usePositionMutations, usePositionsQuery, useUnitMutations, useUnitsQuery, useUsersListQuery } from './hooks'
import { createFormFor, inputFor, layoutItems, renameFormFor, rootOf } from './lib'
import { DELETE_DIALOG } from './model'
...
export const Structure = () => {
  ...
  const handleSubmitPositions = useCallback(
    (assignments: IUnitAssignment[]) => picking && setPositions.mutate({ id: picking.id, assignments }, { onSuccess: () => setPicking(null) }),
  ...
  const handleCreatePosition = useCallback(
    (input: IPositionInput) => positionMutations.create.mutateAsync(input),
  ...
  return (
    <div style={layout} testId="structure__layout">
      ...
      <UnitPositionsPicker
        ...
        positions={positions}
        users={users}
        pending={setPositions.isPending || positionMutations.create.isPending}
        error={setPositions.error?.message ?? positionMutations.create.error?.message}
