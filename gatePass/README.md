...
|---|---|---|
| GET | `/reports/summary?from=ГГГГ-ММ-ДД&to=ГГГГ-ММ-ДД` | Сводка: выдано/активных/отозвано/уникальных посетителей, по дням, топ принимающих |
| GET | `/reports/passes?from&to` | Пропуска за период (до 2000) |
| GET | `/reports/export?from&to` | Файл `.xlsx` (листы «Сводка» и «Пропуска») |

Период не больше 366 дней. В приложении файл сохраняется в папку «Загрузки».
...
к родителю другого типа и циклы. При первом старте создаётся только корневой узел
«Руководство»; должности заводит администратор.
