...
const CHECK_BASE = 103
const BAR_WIDTH = 2
const BAR_HEIGHT = 44

const codesOf = (text: string) => {
  const values = [...text].map((symbol) => symbol.charCodeAt(0) - ASCII_OFFSET)
  const checksum = values.reduce((sum, value, index) => sum + value * (index + 1), START_B) % CHECK_BASE
  return [START_B, ...values, checksum]
}

export const barcodeSvg = (text: string) => {
  const clean = [...text].filter((symbol) => symbol.charCodeAt(0) >= ASCII_OFFSET && symbol.charCodeAt(0) < 127).join('')
  if (clean.length === 0) return ''
  const bits = `${codesOf(clean).map((code) => PATTERNS[code] ?? '').join('')}${STOP}`
  const width = bits.length * BAR_WIDTH
  const bars = [...bits]
    .map((bit, index) =>
      bit === '1' ? `<rect x="${index * BAR_WIDTH}" y="0" width="${BAR_WIDTH}" height="${BAR_HEIGHT}" />` : '',
    )
    .join('')
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${BAR_HEIGHT}" viewBox="0 0 ${width} ${BAR_HEIGHT}" fill="#000">${bars}</svg>`
}
